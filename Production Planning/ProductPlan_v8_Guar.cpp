...
sprintf(outputfilename,"SBARpaper8_NM%d.csv",Version);
		...
		for (t = 1; t <= T; t++){
			for (c = 1; c <= C; c++) {
				for(p = 1; p <= P; p++) {
					...
					if(t>=1){
						OBJ += pow(1/(1+Alpha),(t-1)) * x[t][c][p] * P_cp[c][p] * S_c[c] * pow((1 + IRInc),t);
					}
				}
			}
			...
			for (c = 1; c <= C; c++) {
				for(m = 1; m <= MP; m++) {
					...
					if(t>=1){
						OBJ += - pow(1/(1+Alpha),(t-1)) * CCapPMt_c[c][t] * u[m][c]; 
					}					
				}
			}
			...
			for (c = 1; c <= C; c++) {
				for (p = 1; p <= P; p++) {
					...
					if(t>=1){
						OBJ += - pow(1/(1+Alpha),(t-1)) * (x[t][c][p] * A_cp[c][p] *  CPM_c[c])  * pow((1 + IRInc),t);
					}						
				}
			}
			...
			for (p = 1; p <= P; p++) {
				for (q = 1; q <= P; q++) {
					for (c = 1; c <= C; c++) {
						...
						if(t>=1){
							OBJ += - pow(1/(1+Alpha),(t-1)) * (CTP_c[c] * D_d[c][p][q] * y[t][p][q][c]) * pow((1 + IRInc),t);
						}	
					}
				}
			}
			...
			for (c = 1; c <= C; c++) {
				for(p = 1; p <= P; p++) {
					...
					if(t>=1){
						OBJ += - pow(1/(1+Alpha),(t-1)) * x[t][c][p] * ( (CS_c[c] + CFH_c[c] + (CI * W_ct[c][t]) + CL) * A_cp[c][p]) * pow((1 + IRInc),t); 
					}
				}
			}	
		}
